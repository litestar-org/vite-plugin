...
describe('litestar-vite-plugin', () => {
    ...
    it('handles missing configuration', () => {
        ...
        /* @ts-ignore */
        expect(() => litestar())
            .toThrowError('litestar-vite-plugin: missing configuration.');

        /* eslint-disable-next-line @typescript-eslint/ban-ts-comment */
        /* @ts-ignore */
        expect(() => litestar({}))
            .toThrowError('litestar-vite-plugin: missing configuration for "input".');
    })
    ...
    it('prevents setting an empty publicDirectory', () => {
        expect(() => litestar({ input: 'resources/js/app.js', publicDirectory: '' })[0])
            .toThrowError('publicDirectory must be a subdirectory');
    })

    it('prevents setting an empty buildDirectory', () => {
        expect(() => litestar({ input: 'resources/js/app.js', buildDirectory: '' })[0])
